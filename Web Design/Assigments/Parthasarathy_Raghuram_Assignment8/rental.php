...
if (isset($_POST['submitted'])) {
	...
	if (empty($_POST['PICKUP_LOCATION_CODE'])) {
		$errors[] = '1)You forgot to enter the airport Location Code';
	}
	if (empty($_POST['PICKUP_STREET_ADDRESS'])) {
		$errors[] = '2)You forgot to enter the Street Address';
	}
	if (empty($_POST['PICKUP_CITY_NAME'])) {
		$errors[] = '3)You forgot to enter the pickup city name';
	}
		if (empty($_POST['PICKUP_STATE'])) {
		$errors[] = '4)You forgot to enter the pickup State';
	}
	
	if ($_POST['RENTAL_ONEWAY'] == 'ONEWAY'){
	if (empty($_POST['RETURN_LOCATION_CODE'])) {
		$errors[] = '5)You forgot to enter the return Location Code';
	}
	
	if (empty($_POST['RETURN_STREET_ADDRESS'])) {
		$errors[] = '6)You forgot to enter the return street address Code';
	}
	if (empty($_POST['RETURN_CITY_NAME'])) {
		$errors[] = '7)You forgot to enter the return city name ';
	}
	if (empty($_POST['RETURN_STATE'])) {
		$errors[] = '8)You forgot to enter the return state';
	}
	}

	if (empty($_POST['AIRLINE_CODE'])) {
		$errors[] = '9)You forgot to enter the airline code';
	}
	if (empty($_POST['FLIGHT_NUMBER'])) {
		$errors[] = '10)You forgot to enter the Flight Number';
	}
	 if (empty($_POST['RATE_CODE'])) {
	 	$errors[] = '11)You forgot to enter your Discount Code.';
	 }
	 
	 if ($_POST['CHILD_INFANT_SEAT'] == 'true'){
	if (empty($_POST['CHILD_INFANT_SEAT_QUANTITY'])) {
		$errors[] = '12)You forgot to enter the infant seat quantity';
	 }
	 }
		 if ($_POST['CHILD_SAFETY_SEAT'] == 'true'){
	if (empty($_POST['CHILD_SAFETY_SEAT_QUANTITY'])) {
		$errors[] = '13)You forgot to enter the safety seat quantity';
	 }
	 }
 if ($_POST['CHILD_BOOSTER_SEAT'] == 'true'){
	if (empty($_POST['CHILD_BOOSTER_SEAT_QUANTITY'])) {
		$errors[] = '14)You forgot to enter the booster seat quantity';
	 }
	 }	 
    ...
    $errors[] = '15)Please select a car group.';
	}
	
		$require_one_of = array('CDW_ACCEPT','PAI_ACCEPT','ALI_ACCEPT'); //names of posted checkboxes
		$one_set=false;
	...
	if(!$one_set){
		$errors[] = '16)Please check your optional coverage';
	}
	 if (empty($_POST['FIRST_NAME'])) {
	 	$errors[] = '17)You forgot to enter your First Name.';
	 }
	  if (empty($_POST['LAST_NAME'])) {
	 	$errors[] = '18)You forgot to enter your Last Name.';
	 }
	
	// Check for an email address.
	 if (empty($_POST['EMAIL_ADDRESS'])) {
	 	$errors[] = '19)You forgot to enter your email address.';
	 }
...
<TR>
	<TD width="35%"><INPUT TYPE=checkbox  NAME="RENTAL_ONEWAY"  VALUE="ONEWAY">
